...
namespace RenderFunc {
	...
	template<int d,class T> void Write_Vectors_Float(const fs::path& file_name,const Array<Vector<T,d> >& X,const Array<Vector<T,d> >& V,bool as_ends = false);
	...
	template<int d> void Write_Scalars_As_Points_Float(const fs::path& file_name,const GeometryParticles<d>& particles,const Array<real>& arr,const real scale = 1.0);
	//each segment originates from xs[i],with direction normals[i] and length len_arr[i]*scale
	template<int d> void Write_Customized_Segments_Float(const fs::path& file_name,const Array<Vector<real,d> >& xs,const Array<Vector<real,d> >& normals,const Array<real>& len_arr,const real scale = 1.0);
