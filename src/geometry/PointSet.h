...
public:
	...
	void Update()
	{
		std::cout << "number of dps: " << points->Size() << std::endl;
		Update_Nbs();
	...
	static VectorT Rotate_To_TPlane(const VectorD& u, const MatrixD& e); //same as Project_To_TPlane, but preserves length
	...
	int Nearest_Geometry_KNN(VectorD& pos, MatrixD& frame, int k = 10, int minimum_eqns = 1, bool verbose = true)const;
